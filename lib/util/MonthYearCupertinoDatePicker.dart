...
enum MonthYearCupertinoDatePickerMode {
...
class MonthYearCupertinoDatePicker extends StatefulWidget {
  ...
  MonthYearCupertinoDatePicker({
    Key? key,
    this.mode = MonthYearCupertinoDatePickerMode.date,
    ...
    required this.onDateTimeChanged,
        ...
        assert(minimumYear != null), super(key: key){
    assert(this.initialDateTime != null);
    assert(
    mode != MonthYearCupertinoDatePickerMode.date || (minimumYear >= 1 && this.initialDateTime.year >= minimumYear),
    'initial year is not greater than minimum year, or minimum year is not positive',
    );
    assert(
    mode != MonthYearCupertinoDatePickerMode.date || maximumYear == null || this.initialDateTime.year <= maximumYear!,
    'initial year is not smaller than maximum year',
    );
    assert(
    mode != MonthYearCupertinoDatePickerMode.date || minimumDate == null || !minimumDate!.isAfter(this.initialDateTime),
    'initial date ${this.initialDateTime} is not greater than or equal to minimumDate $minimumDate',
    );
    assert(
    mode != MonthYearCupertinoDatePickerMode.date || maximumDate == null || !maximumDate!.isBefore(this.initialDateTime),
  ...
  final MonthYearCupertinoDatePickerMode mode;
  ...
  State<StatefulWidget> createState() { // ignore: no_logic_in_create_state, https://github.com/flutter/flutter/issues/70499
    ...
    // state.
    switch (mode) {
      case MonthYearCupertinoDatePickerMode.date:
        return _MonthYearCupertinoDatePickerDateState(dateOrder: dateOrder);
    }
  }
...
class _MonthYearCupertinoDatePickerDateState extends State<MonthYearCupertinoDatePicker> {

  _MonthYearCupertinoDatePickerDateState({
  ...
  void _refreshEstimatedColumnWidths() {
    estimatedColumnWidths[_PickerColumnType.month.index] = MonthYearCupertinoDatePicker._getColumnWidth(_PickerColumnType.month, localizations, context);
    estimatedColumnWidths[_PickerColumnType.year.index] = MonthYearCupertinoDatePicker._getColumnWidth(_PickerColumnType.year, localizations, context);
  }
    ...
    return NotificationListener<ScrollNotification>(
      ...
      child: CupertinoPicker.builder(
        ...
        onSelectedItemChanged: (int index) {
          selectedYear = index;
          if (_isCurrentDateValid) {
            widget.onDateTimeChanged(DateTime(selectedYear, selectedMonth, 01));
          }
        },
